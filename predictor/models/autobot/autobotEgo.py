# Autobot: https://github.com/roggirg/AutoBots
# Published May 2nd 2022 under BSD-3-Clause
...
from predictor.models.uncertainty import HeteroscedasticGaussianProcess
...
class OutputModel(nn.Module):
    ...
    def forward(self, agent_decoder_state):
        ...
        return torch.stack([x_mean, y_mean, pred_obs[:,:,2], pred_obs[:,:,3], pred_obs[:,:,4]], dim=2)
...
class AutoBotEgo(BaseModel):
    ...
    def __init__(self, config, k_attr=2, map_attr=2):
        ...
        #self.learning_rate_schedule= config.trainer.learning_rate_sched
        self.epsilon = self.config.trainer.epsilon
        self.T = self.config.data['future_len']
        self.past = self.config.data['past_len']
        ...
        self.tx_hidden_size = self.config.model['tx_hidden_size']
        ...
        if self.uncertainty:
            self.prob_predictor = HeteroscedasticGaussianProcess(
                ...
                likelihood = self.config.model.gp_likelihood,
            ).double()
        else:
    ...
    def _forward(self, inputs):
        ...
        :param roads_in: [B, S, P, map_attr+1] representing the road network if self.use_map_lanes or
        ...
        :return:
            dictionary:
                predicted_trajectory: shape [B, c, T, 5] c trajectories for the ego agents with every point being the params of
                                            Bivariate Gaussian distribution.
                predicted_probability: shape [B, c] mode probability predictions P(z|X_{1:T_obs})
        '''
        ...
        if not self.uncertainty:
            logits = self.prob_predictor(mode_params_emb).squeeze(-1).transpose(0, 1)
            mode_probs = F.softmax(logits, dim=-1)
            mode_log_probs = F.log_softmax(logits, dim=-1)
        else:
            with autocast(device_type="cuda" if mode_params_emb.is_cuda else "cpu", enabled=False):
                mode_params_emb = mode_params_emb.transpose(0, 1)
                combined_mode_params_emb = mode_params_emb.reshape(B, -1)
                logits, pred_mean, pred_variance, gp_pred_variance = self.prob_predictor(combined_mode_params_emb.double())
                mode_probs = pred_mean.to(out_dists.dtype)
                mode_log_probs = logits.to(out_dists.dtype)
                output['predictive_uncertainty'] = pred_variance.to(out_dists.dtype)
                output['epistemic_uncertainty'] = gp_pred_variance.to(out_dists.dtype)
        ...
        if not self.training:
            output['agent_embedding'] = agents_emb
            output['map_embedding'] = orig_map_features
            output['scene_context'] = context
            output['scene_decoding'] = out_seq
    ...
    def configure_optimizers(self):
        # optimizer = optim.Adam(self.parameters(), lr=self.learning_rate, eps=0.0001)
        # scheduler = MultiStepLR(optimizer, milestones=self.learning_rate_schedule, gamma=0.5,
        #                         verbose=True)
        # optimizer = RAdam(
        #     self.parameters(), 
        #     lr=self.learning_rate, 
        #     eps=self.epsilon, 
        #     weight_decay=1e-2, # default weight decay in AdamW
        #     decoupled_weight_decay=True # activates AdamW behavior
        # )
...
class Criterion(nn.Module):
    ...
    def get_BVG_distributions(self, pred):
        ...
        sigma_x = torch.exp(torch.clip(pred[:, :, 2], min=self.log_std_range[0], max=self.log_std_range[1]))
        sigma_y = torch.exp(torch.clip(pred[:, :, 3], min=self.log_std_range[0], max=self.log_std_range[1]))
        rho = torch.clip(pred[:, :, 4], min=-0.5, max=0.5)

        cov = torch.zeros((B, T, 2, 2), device=pred.device)
        cov[:, :, 0, 0] = sigma_x ** 2
        cov[:, :, 1, 1] = sigma_y ** 2
    ...
    def get_Laplace_dist(self, pred):
        sigmas=torch.exp(torch.clip(pred[:, :, 2:4], min=self.log_std_range[0], max=self.log_std_range[1]))
        return Laplace(pred[:, :, :2], sigmas)

    def nll_pytorch_dist(self, pred, data, mask, rtn_laplace=True):
        # Note: Laplace is independent of rho but has longer tails
        if rtn_laplace:
            biv_gauss_dist = self.get_Laplace_dist(pred)
        else:
            biv_gauss_dist = self.get_BVG_distributions(pred)
        data_reshaped = data[:, :, :2]  # shape: [B, T, 2]
        log_prob = biv_gauss_dist.log_prob(data_reshaped)  # shape: [B, T, 2] in case of Laplace, [B, T] in case of gaussian
        if rtn_laplace:
            # Multiply by the mask and sum over timesteps to get loss per sample.
            return ((-log_prob).sum(dim=-1) * mask).sum(dim=1)  # shape: [B]
        else:
            # Return the per-timestep negative log likelihood, masked appropriately.
            return (-log_prob * mask).sum(dim=1)  # shape: [B]
    ...
    def nll_loss_multimodes(self, output, data, center_gt_final_valid_idx):
        ...
        Args:
          ...
          noise is optional
        """
        modes_pred = output['predicted_log_probability']
        pred = output['predicted_trajectory'].permute(1, 2, 0, 3)
        mask = data[..., -1]

        entropy_weight = self.config.model['entropy_weight']
        kl_weight = self.config.model['kl_weight']
        use_FDEADE_aux_loss = self.config.model['use_FDEADE_aux_loss']
        ...
        with torch.no_grad():
            log_lik = torch.zeros((batch_sz, modes), device=data.device)
            for kk in range(modes):
                nll = self.nll_pytorch_dist(pred[kk].transpose(0, 1), data, mask, rtn_laplace=self.config.model["laplace_nll"])
                #log_lik[:, kk] = -nll.cpu().numpy()
                log_lik[:, kk] = -nll.detach()

            priors = modes_pred.detach()
            log_posterior_unnorm = log_lik + priors
            log_posterior = log_posterior_unnorm - torch.logsumexp(log_posterior_unnorm, axis=-1, keepdim=True)
            post_pr = torch.exp(log_posterior)

        # Compute loss.
        loss = 0.0
        for kk in range(modes):
            nll_k = self.nll_pytorch_dist(pred[kk].transpose(0, 1), data, mask, rtn_laplace=self.config.model["laplace_nll"]) * post_pr[:, kk]
            loss += nll_k.mean()

        # Adding entropy loss term to ensure that individual predictions do not try to cover multiple modes.
        entropy_vals = []
        for kk in range(modes):
            entropy_vals.append(self.get_BVG_distributions(pred[kk]).entropy() * mask.transpose(0,1)) # CUSTOM: added mask to not count invalid points
        entropy_vals = torch.stack(entropy_vals).permute(2, 0, 1)
        entropy_loss = torch.mean((entropy_vals).sum(2).max(1)[0]) - 2*entropy_vals.shape[-1] # CUSTOM: added this recentering to prevent collapse
        loss += entropy_weight * torch.clip(entropy_loss, 0, 100)
        ...
        kl_loss = kl_weight * kl_loss_fn(modes_pred, post_pr)
        ...
        if use_FDEADE_aux_loss:
            ...
            adefde_loss = torch.tensor(0.0, device=data.device)
