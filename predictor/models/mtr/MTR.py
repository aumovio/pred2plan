...
import predictor.models.mtr.loss_utils as loss_utils
import predictor.models.mtr.motion_utils as motion_utils
from predictor.models.base_model.base_model import BaseModel
from predictor.models.mtr.MTR_utils import PointNetPolylineEncoder, get_batch_offsets, build_mlps
#from predictor.models.mtr.ops.knn import knn_utils
import predictor.models.mtr.ops.knn.non_cuda as knn_utils
from predictor.models.mtr.transformer import transformer_decoder_layer, position_encoding_utils, \
...
class MotionTransformer(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.model_cfg = EasyDict(self.config.model)
        self.pred_dicts = []
        self.learning_rate = self.config.trainer["learning_rate"]

        # self.model_cfg.MOTION_DECODER['CENTER_OFFSET_OF_MAP'] = self.config.data['center_offset_of_map']
        # self.model_cfg.MOTION_DECODER['NUM_FUTURE_FRAMES'] = self.config.data['future_len']
        # self.model_cfg.MOTION_DECODER['OBJECT_TYPE'] = self.config.data['object_type']
    ...
    def forward(self, batch):
        ...
        if self.training:
            output['predicted_probability'] = F.softmax(mode_probs, dim=-1)  # #[B, c]
            output["predicted_log_probability"] = F.log_softmax(mode_probs, dim=-1) # [B, c]
            ...
            output["predicted_log_probability"] = torch.log(out_dict["pred_scores"]) # [B, c]
            output['predicted_trajectory'] = out_dict['pred_trajs']  # [B, c, T, 5] to be able to parallelize code
            output['scene_context'] = out_dict['scene_context']
            output['agent_embedding'] = enc_dict["obj_feature"]
            output['map_embedding'] = enc_dict["map_feature"]
    ...
    def configure_optimizers(self):
        # decay_steps = [x for x in self.config.trainer['learning_rate_sched']]

        # def lr_lbmd(cur_epoch):
        #     cur_decay = 1
        #     for decay_step in decay_steps:
        #         if cur_epoch >= decay_step:
        #             cur_decay = cur_decay * self.config.trainer['lr_decay']
        #     return max(cur_decay, self.config.trainer['lr_clip'] / self.config.trainer['learning_rate'])

        # optimizer = torch.optim.AdamW(self.parameters(), lr=self.config.trainer['learning_rate'],
        #                               weight_decay=self.config.trainer['weight_decay'], eps=self.config.trainer["epsilon"])
...
class MTREncoder(nn.Module):
    ...
    def apply_global_attn(self, x, x_mask, x_pos):
        ...
        pos_embedding = position_encoding_utils.gen_sineembed_for_position(x_pos_t[...,0:2], hidden_dim=d_model)
...
class MTRDecoder(nn.Module):
    ...
    def get_motion_query(self, center_objects_type):
        ...
        if self.use_place_holder:
            ...
            intention_query = position_encoding_utils.gen_sineembed_for_position(intention_points[...,0:2],
    ...
    def apply_cross_attention(self, kv_feature, kv_mask, kv_pos, query_content, query_embed, attention_layer,
        ...
        searching_query = position_encoding_utils.gen_sineembed_for_position(dynamic_query_center[...,0:2], hidden_dim=d_model)
        kv_pos = kv_pos.permute(1, 0, 2)[:, :, 0:2]
        kv_pos_embed = position_encoding_utils.gen_sineembed_for_position(kv_pos[...,0:2], hidden_dim=d_model)
    ...
    def apply_transformer_decoder(self, center_objects_feature, center_objects_type, obj_feature, obj_mask, obj_pos,
        ...
        return pred_list, query_content
    ...
    def generate_final_prediction(self, pred_list, batch_dict):
        ...
        pred_scores_final = pred_scores_final / pred_scores_final.sum(dim=-1, keepdim=True)
    ...
    def forward(self, batch_dict):
        ...
        # decoder layers
        pred_list, query_content = self.apply_transformer_decoder(
        ...
        if not self.training:
            ...
            batch_dict['scene_context'] = query_content
