...
from simulation.utils.scenarionet.nuplan.type import get_traffic_obj_type, NuPlanEgoType, set_light_status
from simulation.utils.scenarionet.utils import nuplan_to_metadrive_vector, compute_angular_velocity
...
try:
    ...
    NUPLAN_PACKAGE_PATH = nuplan.__file__[0] #os.path.dirname(nuplan.__file__)
...
def convert_nuplan_scenario(scenario: NuPlanScenario, version):
    ...
    result[SD.METADATA]["scenario_center"] = scenario_center
    # align it with other result dictionaries from other converters
    result[SD.METADATA]["track_length"] = result["tracks"][EGO]
    result[SD.METADATA]["source_file"] = scenario.log_name
...
#     reference_frame = 49
#     obj_types = [obj["type"] for obj in tracks.values()]
#     validities = [sum(obj["state"]["valid"]) for obj in tracks.values()]
#     velocities = [np.mean(obj["state"]["velocity"]) for obj in tracks.values()]
#     for obj in tracks.values():
#         if obj["type"] != "VEHICLE":
#             continue
#         if sum(obj["state"]["valid"]) < 0.9 * obj["metadata"]["track_length"]:
#             continue
#         if np.mean(obj["state"]["velocity"]) < 10:
#             continue
#     return None
